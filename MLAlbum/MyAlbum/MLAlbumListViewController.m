...
- (void)creatNav {
    ...
    self.navigationController.navigationBar.barTintColor = [UIhelpTools colorWithHexString:@"333333"];
    ...
    [ [UIButton alloc]initWithFrame:CGRectMake(8,0,40,22) ];
    [leftBarItemCustomButton setTitle:@"取消" forState:UIControlStateNormal];
    leftBarItemCustomButton.titleLabel.font = [UIFont systemFontOfSize:12];
...
- (void)loadData {
    [GetImageGroups getGroupsWithArrayBlock:^(NSMutableArray *imageArray, ALAssetsLibrary *library, NSString *error) {
        ...
        if (error.length != 0) {
            UIAlertView *aler = [[UIAlertView alloc]initWithTitle:@"提示" message:error delegate:self cancelButtonTitle:@"好滴好滴" otherButtonTitles: nil];
            [aler show];
        }
        [table reloadData];
    }];
