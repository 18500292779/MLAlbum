...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        _nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(ViewEndX(_image)+10, 10, SCREEN_WIDTH - (ViewEndX(_image)+10+12.5) , 16)];
        _nameLabel.textColor = [UIhelpTools colorWithHexString:@"444444"];
        _nameLabel.font = [UIFont systemFontOfSize:16];
        [self addSubview:_nameLabel];
        
        _numberLabel = [[UILabel alloc]initWithFrame:CGRectMake(ViewEndX(_image)+10, ViewEndH(_nameLabel)+15, SCREEN_WIDTH - (ViewEndX(_image)+10+12.5) , 16)];
